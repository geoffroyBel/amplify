...
import { switchUser, updateProfile, deleteProfile } from "./actions/awsAuth";
//import { fetchUserCompany } from "./actions/companyActions";
...
export default class AmplifyBridge {
	...
	checkUserSuccess(user) {
		...
		//switchUser(user)(dispatch)
		this.store.dispatch(switchUser(user));
		// this.store.dispatch(fetchAllUserAbonnements(user));
		// this.store.dispatch(fetchUserCompany(user));
	...
	translateAttributes(data) {
		...
		data
			.filter((attr) =>
				["given_name", "family_name", "email", "phone_number"].includes(
					attr.Name
				)
			)
